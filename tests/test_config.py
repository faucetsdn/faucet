...
class DistConfigTestCase(unittest.TestCase):
    ...
    def test_stacking(self):
        ...
        self.assertEqual(
            list(edge_data_a.values()), list(edge_data_b.values()))
    ...
    def test_ports_vlans(self):
        for dp in (self.v2_dp,):
            ...
            self.assertEqual(dp.get_native_vlan(2), vlan_40)
            self.assertEqual(dp.get_native_vlan(3), vlan_40)
            self.assertEqual(dp.get_native_vlan(4), vlan_41)
            self.assertEqual(dp.get_native_vlan(5), vlan_41)
    ...
    def test_routing(self):
        for dp in (self.v2_dp,):
            vlan = dp.vlans[41]
            self.assertIn(
                ipaddress.ip_interface('10.0.0.253/24'),
                vlan.faucet_vips
                )
