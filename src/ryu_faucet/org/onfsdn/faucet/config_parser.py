...
def _dp_include(config_hashes, config_file, logname, top_confs):
    ...
    new_top_confs = {}
    for top_conf_name, top_conf in top_confs.iteritems():
        new_top_confs[top_conf_name] = top_conf.copy()
    ...
    for top_conf_name, top_conf in top_confs.iteritems():
        new_top_confs[top_conf_name].update(conf.pop(top_conf_name, {}))
    ...
    for top_conf_name, top_conf in top_confs.iteritems():
        top_confs[top_conf_name].update(new_top_confs[top_conf_name])
...
def _dp_parser_v2(config_file, logname):
    ...
    if not _dp_include(config_hashes, config_file, logname, top_confs):
    ...
    for identifier, dp_conf in top_confs['dps'].iteritems():
        ...
        for vid, vlan_conf in top_confs['vlans'].iteritems():
        ...
        for port in ports.itervalues():
            dp.add_port(port)
        for a_identifier, acl_conf in top_confs['acls'].iteritems():
