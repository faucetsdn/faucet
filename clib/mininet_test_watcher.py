...
class TopologyWatcher():
    ...
    def get_connected_hosts(self, two_way=False):
        """
        ...
        for src_host in self.host_name_to_index:
            ...
            for dst_host in self.host_name_to_index:
                ...
                if src_index != dst_index and self.is_shared_router(src_index, dst_index):
                    #if not two_way or (dst_index not in connected_hosts and src_index not in connected_hosts[dst_index]):
                    if two_way or (
                            src_index not in connected_hosts[dst_index] and dst_index not in connected_hosts[src_index]):
                        try:
