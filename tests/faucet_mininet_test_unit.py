    ...
    def test_untagged(self):
        ...
        self.assertEqual(
            ...
            self.scrape_prometheus_var(
                'dp_status', labels=labels, controller='gauge', retries=3))
        port_labels = labels.update(
            {'port_name': self.port_map['port_1']})
        last_p1_bytes_in = 0
        for _ in range(2):
            ...
            for _ in range(self.DB_TIMEOUT * 3):
                ...
                p1_bytes_in = self.scrape_prometheus_var(
                    'of_port_rx_bytes', labels=port_labels, controller='gauge', retries=3)
